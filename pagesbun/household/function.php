...
if(isset($_POST['btn_add'])){
    ...
    $chkdup = mysqli_query($con, "SELECT * from tblhouseholdbun where householdno = ".$txt_householdno."");
    ...
    if($rows == 0){
        $query = mysqli_query($con,"INSERT INTO tblhouseholdbun (householdno, totalhouseholdmembers, headoffamily, purok) 
...
if (isset($_POST['btn_save'])) {
    ...
    $result = mysqli_query($con, "SHOW COLUMNS FROM tblhouseholdbun");
    $valid_columns = [];
    ...
    if (in_array('householdno', $valid_columns) && in_array('totalhouseholdmembers', $valid_columns) && in_array('purok', $valid_columns)) {
        ...
        $stmt = $con->prepare("UPDATE tblhouseholdbun SET householdno = ?, totalhouseholdmembers = ?, purok = ? WHERE id = ?");
...
if(isset($_POST['btn_delete']))
{
    if(isset($_POST['chk_delete']))
    {
        ...
        {
            $delete_query = mysqli_query($con,"DELETE from tblhouseholdbun where id = '$value' ") or die('Error: ' . mysqli_error($con));
